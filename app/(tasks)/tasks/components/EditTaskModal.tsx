...
import type { Task, TaskStatus } from "@/lib/generated/prisma";
import {
  Dialog,
  DialogHeader,
  DialogTitle,
...
interface EditTaskModalProps {
  open: boolean;
  onClose: () => void;
  onUpdateTask: (taskId: number, taskData: Task) => void;
  task: Task | null;
}
...
export function EditTaskModal({
  ...
  useEffect(() => {
    if (task) {
      const deadlineString = task.deadline?.toISOString().slice(0, 16);
      setFormData({
        title: task.title,

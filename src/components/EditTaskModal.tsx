...
import type { Task, UpdateTaskData, TaskStatus } from "@/types/task";
...
interface EditTaskModalProps {
  open: boolean;
  onClose: () => void;
  onUpdateTask: (taskId: string, taskData: UpdateTaskData) => void;
  task: Task | null;
}

export function EditTaskModal({ open, onClose, onUpdateTask, task }: EditTaskModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.deadline) {
      ...
      if (deadlineDate <= now && formData.status !== 'completed') {
        newErrors.deadline = 'La fecha límite debe ser futura para tareas no completadas';
      }
    }
